          ...
          {data.map((education, index) => (
            <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
              ...
              <div className='grid md:grid-cols-2 gap-3'>
                ...
                <input value={education.graduation_date || ""} onChange={(e) => updateEducation(index, "graduation_date", e.target.value)} type='month' disabled={education.is_current} className='px-3 py-2 text-sm ' />
              </div>

              <input value={education.cgpa || ""} onChange={(e) => updateEducation(index, "cgpa", e.target.value)} type='number' className='px-3 py-2 text-sm' placeholder='cgpa (optional)' />
