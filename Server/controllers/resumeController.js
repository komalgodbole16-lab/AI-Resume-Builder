...
export const createResume = async (req, res) => {
    try {
        ...
        // create new resume
        const newResume = await Resume.create({userId, title, ...resumeDataCopy});
...
resumeDataCopy.personal_info.image = response.url
       ...
       const updatedResume =  await Resume.findOneAndUpdate({userId, _id: resumeId}, 
        {$set: resumeDataCopy}, 
        {new: true})

       return res.status(200).json({message: 'saved successfuly', resume : updatedResume})
    } catch (error) {
        return res.status(400).json({message: error.message})
    }
}
